#ifndef JL_FUNCTION_MOVE_ONLY
#define JL_FUNCTION_MOVE_ONLY
#include <array>
#include <cstddef>
#include <memory>
#include <stdexcept>
...
namespace jl::move_only {
    ...
    public:
        ...
        explicit callable(Impl const& impl) = delete;
    ...
    public:
        ...
        R operator()(Args &&... args) {
            if (stored_function == nullptr) {
                throw std::runtime_error("Bad function call in jl::move_only::type_erased_callable");
            }

            return (*stored_function)(static_cast<Args&&>(args)...);
        }

        template<concepts::move_only_callable<R, Args...> Impl>
        ...
        type_erased_callable(this_t&& other) noexcept : stored_function(nullptr) {
            *this = static_cast<this_t&&>(other);
        }

        this_t& operator=(this_t const&) = delete;
        type_erased_callable(this_t const& other) = delete;
